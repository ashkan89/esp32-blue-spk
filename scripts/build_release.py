...
def main():
    ...
    parser.add_argument('--out',type=Path,default=ROOT/'dist');parser.add_argument('--skip-build',action='store_true');args=parser.parse_args()
    ...
    version=re.search(r'#define FW_VERSION\s+"([^"]+)"',(ROOT/'src/app_config.h').read_text()).group(1)
    targets={'wroom':'esp32_wroom_32d_16mb_release','wrover':'esp32_wrover_e_n16r8_release'}
    if not args.skip_build:
        subprocess.run([sys.executable,'-m','platformio','run','-j','3',*[item for env in targets.values() for item in ('-e',env)]],cwd=ROOT,check=True)
